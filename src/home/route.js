...
function mapFilesIntoBody(req, _res, next) {
    ...
    if (logoFile) {
        const web = toWeb(logoFile);
        if (web) req.body.logoImage = web;
    }
    ...
    for (const f of req.files) {
        ...
        if (web) {
            ...
            req.body.brandsPreviewImage[idx] = web;
        }
    }
    ...
    if (brandFiles && brandFiles.length > 0 && req.body.brandsPreviewImage.length === 0) {
        req.body.brandsPreviewImage = brandFiles.map(toWeb).filter(Boolean);
    }
